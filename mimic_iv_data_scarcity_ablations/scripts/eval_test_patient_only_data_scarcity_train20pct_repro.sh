...
#
# Prerequisites:
#   sweep_patient_only_data_scarcity_n25_30d_train20pct.sh must have
#   already finished (checkpoints at
#   outputs/patient_only_data_scarcity_n25_30d_train20pct/draw<d>/checkpoints/last.ckpt).
#
# Usage:
#   cd mimic_iv_data_scarcity_ablations
#   sbatch scripts/eval_test_patient_only_data_scarcity_train20pct_repro.sh
# ============================================================

#SBATCH --job-name=mimic-ds-abl-patient-only-repro-train20pct
#SBATCH --array=0-4
...
TRAIN_OUTPUT_DIR="/groups/mm6677_gp/hs3627/medrap-experiments/mimic_iv_sweep/outputs/patient_only_data_scarcity_n25_30d_train20pct/draw${DRAW}"
CHECKPOINT_PATH="${TRAIN_OUTPUT_DIR}/checkpoints/last.ckpt"
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_data_scarcity_n25_30d_train20pct_test_repro/draw${DRAW}"
...
echo "  Duration      : 30d"
echo "  Draw          : ${DRAW}/5"
echo "  Architecture  : patient_only (no retrieval), FULL capacity, train20pct"
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=mimic-ds-abl-patient-only-repro-train20pct-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
