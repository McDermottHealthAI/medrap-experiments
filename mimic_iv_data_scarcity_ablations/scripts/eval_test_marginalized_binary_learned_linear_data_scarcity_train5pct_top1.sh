...
# (retriever.k=1, ablation_mode=none) for train5pct, alongside the
# existing repro/random/null scripts.
# ============================================================

#SBATCH --job-name=mimic-ds-abl-marginalized-top1-train5pct
#SBATCH --array=0-4
...
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_data_scarcity_n25_30d_train5pct_test_eval_top1/draw${DRAW}"
...
echo "  Draw          : ${DRAW}/5"
echo "  query_projector: sequence_mean_1024 (learned-linear), FULL capacity, train5pct, top1-only"
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=mimic-ds-abl-marginalized-top1-train5pct-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
