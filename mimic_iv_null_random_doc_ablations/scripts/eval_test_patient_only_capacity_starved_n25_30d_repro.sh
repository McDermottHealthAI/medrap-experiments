...
#SBATCH --job-name=mimic-repro-eval-test-patient-only-capacity-starved-n25-30d
#SBATCH --array=0-4
...
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_capacity_starved_n25_30d_test_repro/draw${DRAW}"
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=mimic-repro-patient-only-capacity-starved-test-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
