#!/bin/bash
# ============================================================
# MIMIC ablation-line script: evaluates HAYK's ORIGINAL capacity-starved
# checkpoints (hs3627 working copy, read-only) against the snapshot-restored
# original tensorized cohort. Adds a top-1-only inference-style eval
# (retriever.k=1, ablation_mode=none) alongside the existing repro/random/null
# scripts -- retriever.k=1 is valid for any checkpoint regardless of its
# trained k (PerDocCrossAttentionFusion and marginalized_output_mode=binary
# marginalization are both K-agnostic).
# ============================================================
# ============================================================
# SLURM array: held-out TEST split AUROC, TOP-1 DOC ONLY, for the
#              capacity-starved marginalized(binary, learned-linear query
#              projector) checkpoints.
# ------------------------------------------------------------
# retriever.k=1, ablation_mode=none: only the single nearest-neighbor
# (highest-scoring) retrieved document, at eval time. Compares against the
# same checkpoint's real-docs (k=4) repro number.
#
# Prerequisites:
...
#   sbatch scripts/eval_test_marginalized_binary_learned_linear_capacity_starved_n25_30d_top1.sh
# ============================================================

#SBATCH --job-name=mimic-abl-eval-test-marginalized-binary-learned-linear-capacity-starved-n25-30d-top1
#SBATCH --array=0-4
...
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_capacity_starved_n25_30d_test_eval_top1/draw${DRAW}"
...
echo "  Draw          : ${DRAW}/5"
echo "  query_projector: sequence_mean_1024 (learned-linear), CAPACITY-STARVED, top1-only"
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=mimic-abl-marginalized-learned-linear-capacity-starved-top1-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
