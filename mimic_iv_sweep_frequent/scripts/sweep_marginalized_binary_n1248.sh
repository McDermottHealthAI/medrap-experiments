...
#              same task labels/architecture as
#              sweep_marginalized_n1248.sh, testing the
#              MedRAP#93 (branch fix/marginalized-binary-output-mode) fix
...
# forcing them to compete for probability mass. This produced the bimodal
# near-0/near-1 per-task AUROC seen in marginalized-frequent-n{N}-* on W&B.
# marginalized_output_mode=binary uses an independent per-task sigmoid
# marginalized over documents only (never over tasks) instead -- see
# MedRAP PR #93 for the full writeup, including a doctest proving this is
# numerically identical to what MultiTaskBCEMarginalizedLoss already trains
# against (so training/loss should be unaffected; only the metrics/logits
# built on top of it should change).
#
...
# task's AUROC by up to ~0.5 -- as large as every marginalized-vs-
# patient_only gap this experiment has ever reported, which is why the
# headline table it feeds cannot be trusted. Numbers from this script are
...
# degree of freedom. Note this changes the trained weights, so checkpoints
# from this script are only comparable to eval runs that also pass
...
#SBATCH --job-name=sweep-marginalized-binary-n1248-frequent
#SBATCH --array=0-7
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-binary-frequent-n${N}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
