...
# (two validation passes per epoch instead of five -- 12 over the 6 epochs
# here, not 30). That prefix held so few positives per task that a single
# positive changing rank could move the task's AUROC by up to ~0.5, which is
# the same size as the 6-epoch-vs-3-epoch difference this script exists to
# measure. Numbers from this script are therefore NOT comparable to results
# published before this change; note the full-split validation also costs
# real wall-clock time on top of the doubled epoch budget, so watch the
# 10:00:00 --time ceiling.
#
