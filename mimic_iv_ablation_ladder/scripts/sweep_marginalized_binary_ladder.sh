...
#SBATCH --job-name=sweep-marginalized-binary-ladder
#SBATCH --array=0-79
...
OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_ladder/${SELECTION}/seed${MODEL_SEED}/n${N}"
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-binary-ladder-${SELECTION}-seed${MODEL_SEED}-n${N}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
