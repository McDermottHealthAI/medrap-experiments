...
#              and RANDOM code selection (code_selection=random) for
#              N in {1, 2, 4, 8, 16, 32, 64, 128} -- the `random` half of the
#              ablation ladder's two-table label set.
# ------------------------------------------------------------
# This is the label source for SEL_IDX=0 in every training and eval script in
# this directory. Its sibling, generate_labels_frequent.sh, produces SEL_IDX=1.
# Run both before the sweep; run this one first, since the plan's execution
# order puts the `random` table ahead of the `most_frequent` table.
#
...
#    7-day horizon -- positive rate 0.0022 -> 0.2057, a ~92x increase. Under
#    random selection the anchored median in-window positive rate is ~0.026,
#    which is the number that applies to THIS script (see the gate below).
#    Also measured on that shard: 554/554 anchors land on a real clinical-event
...
#    calendar anchor).
#
# 2. code_selection=random is passed EXPLICITLY. It is also the
#    _preprocess.yaml default, so this line is redundant today -- that is
#    deliberate. This directory generates two label trees that differ in
#    exactly this key, the arm is identified by SEL_IDX in eight downstream
#    scripts, and a future default flip upstream must not silently move this
#    arm. ../mimic_iv_sweep/scripts/generate_labels_anchored_n1248.sh is the
#    counterexample: it relies on the default and then gates as if it had
...
# ../mimic_iv_sweep_frequent, or anywhere else. Positive rates, n_valid_tasks,
# and any AUROC computed on them do not belong on the same axis as a published
# uniform_lifetime run. Compare anchored to anchored only; a headline number
# mixing the two is measuring the anchor, not the model.
#
...
#   data/tasks_random/n<N>/tasks/{train,tuning,held_out}.parquet
#
...
#   sbatch --array=2 scripts/generate_labels_random.sh   # only N=4
# ============================================================

#SBATCH --job-name=ladder-gen-labels-random
#SBATCH --array=0-7
...
OUTPUT_DIR="${REPO_DIR}/data/tasks_random/n${N}"
...
echo "  N (tasks)      : ${N}"
echo "  Code selection : random (explicit, not the default)"
...
# affordable only because of anchor_strategy=uniform_event, and it is set
# deliberately BELOW the projected per-task minimum under random selection
# (~38 positives; median 544, at ~0.026 in-window positive rate). That margin
# is the whole point: 25 catches a genuine failure of the anchoring or of the
# label join, without aborting an 8-job array over a single legitimately rare
# code that the random draw happened to pick.
#
