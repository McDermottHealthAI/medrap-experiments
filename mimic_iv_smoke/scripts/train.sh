...
#     mimic_iv/ — no need to re-run medrap-preprocess's MEDS-transforms /
#     MTD_preprocess stages.
#   - task labels are generated by scripts/generate_tasks.sh (the new
#     `medrap-preprocess` / task_generation stage), not reused from
#     ../mimic_iv, so this smoke test actually exercises that code path.
...
MT_LABELS_DIR="${MT_LABELS_DIR:-${REPO_DIR}/data/tasks_gen/tasks}"
NUM_TASKS=25
