...
## Capacity-starved patient_only vs. marginalized retrieval (N=25, 30d) (`sweep_patient_only_capacity_starved_n25_30d.sh` + `sweep_marginalized_binary_qwen3_text_capacity_starved_n25_30d.sh` + `sweep_marginalized_binary_learned_linear_capacity_starved_n25_30d.sh`)

**Status: COMPLETE -- all 15 jobs (5 draws x 3 architectures) finished. First consistent, statistically-larger-than-noise retrieval benefit found in this repo.**

### The model, layer by layer

Every experiment in this README (this section included) trains the same
underlying architecture on MIMIC-IV, in one of two configurations
(`patient_only` or `marginalized`/retrieval-augmented). Below is the exact
sequence of layers each configuration runs, in forward-pass order.

**Task**: multi-task binary classification -- for N=25 randomly-sampled
diagnostic codes, predict whether each will occur within a fixed
occurrence window (7 or 30 days, depending on the section) after a
prediction "anchor" time (see the anchor-sampling sections above for how
that anchor point is chosen). One model, 25 independent binary
...
- **Query projector**: turns the patient's encoded state into a vector
  ...
  in this repo (see "Two query-projector designs" below).
...
document corpus's embeddings. Two designs are compared throughout this
README:
...
Every retrieval variant tried in this repo so far -- the original learned
linear query projector, the Qwen3-aligned frozen one, and a trainable
residual adapter on top of the latter -- has failed to make `marginalized`
*consistently* beat `patient_only` at full model capacity (see all
sections above). This tests a different hypothesis: maybe retrieval only
helps once the base model is too small to represent the task on its own
from patient history alone. If retrieved content carries real information,
a severely **capacity-starved** `patient_only` should degrade much more
...
| Param | Full capacity (every other section) | Starved (this section) |
| --- | --- | --- |
...
### Results (final -- all 15 jobs complete)
...
(from the "Anchor-sampling fix rerun" section above) -- so this ~8x
capacity cut drops mean AUROC by ~0.055 and roughly doubles draw-to-draw
variance (std 0.022 -> 0.035), confirming the starvation is real and
meaningful, not so mild it's within full-capacity noise.
...
| --- | --- | --- |
| learned-linear (`sequence_mean_1024`) | -0.0083 ± 0.0155, **1/5** (PR#13) | **+0.0125 ± 0.0104, 5/5** |
| `qwen3_text` (MedRAP#101) | -0.0013 ± 0.0097, **3/5** (PR#14) | **+0.0248 ± 0.0119, 5/5** |
...
retrieval experiment in this repo failed to show a consistent benefit:
not because the retrieved content is useless (it isn't), but because the
full-capacity patient encoder didn't need it.
...
ablation infrastructure from the "Inference-style evaluation" section
above) -- if starved-random-docs also beats starved-`patient_only` by a
similar margin, the effect is ensembling/regularization, not retrieval
content. If starved-random-docs does *not* show this benefit, that
confirms the retrieved content itself is what's driving the gain.
