...
# (embedding_dim=32, ff_dim=64, num_heads=1, num_layers=1) with
...
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/sweep_patient_only_capacity_sweep_n25_30d_dim32.sh
# ============================================================

#SBATCH --job-name=sweep-patient-only-capacity-sweep-dim32
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_capacity_sweep_n25_30d_dim32/draw${DRAW}"
...
echo "=== Job info ==="
echo "  Draw          : ${DRAW}/5"
echo "  Architecture  : patient_only, embedding_dim=32, max_seq_len=256 (fixed)"
echo "  Started       : $(date)"
echo ""
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=patient-only-capacity-sweep-dim32-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
