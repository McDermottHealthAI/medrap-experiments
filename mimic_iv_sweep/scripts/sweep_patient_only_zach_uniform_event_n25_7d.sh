...
#              labels (see generate_labels_zach_uniform_event_n25_7d.sh) --
#              fixed N=25, 5 draws, 7-day fixed occurrence window.
...
#              7d arm -- this is the "old real-event fix" vs. "Zach's refined
...
#SBATCH --job-name=sweep-patient-only-zach-uniform-event-n25-7d
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_zach_uniform_event_n25_7d/draw${DRAW}"
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=patient-only-zach-uniform-event-d7-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
