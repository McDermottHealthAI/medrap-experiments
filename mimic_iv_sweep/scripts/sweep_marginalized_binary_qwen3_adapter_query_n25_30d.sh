...
#              -- same Zach anchor_strategy=uniform_event 30d labels as
#              sweep_marginalized_binary_qwen3_text_query_n25_30d.sh, only
#              the query projector changes.
# ------------------------------------------------------------
# sweep_marginalized_binary_qwen3_text_query_n25_30d.sh (MedRAP#101) fixed
...
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/sweep_marginalized_binary_qwen3_adapter_query_n25_30d.sh
# ============================================================

#SBATCH --job-name=sweep-marginalized-binary-qwen3-adapter-query-n25-30d
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_qwen3_adapter_query_n25_30d/draw${DRAW}"
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-binary-qwen3-adapter-query-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
