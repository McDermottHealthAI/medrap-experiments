...
#SBATCH --job-name=eval-test-patient-only-task-count-n8
...
TRAIN_OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_task_count_ablation_n8"
CHECKPOINT_PATH="${TRAIN_OUTPUT_DIR}/checkpoints/last.ckpt"
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_task_count_ablation_n8_test_eval"
...
echo "=== Job info ==="
echo "  eval_mode     : test (MEDS held_out split), N=8 tasks"
echo "  Started       : $(date)"
echo ""
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=eval-test-patient-only-task-count-n8-extreme-starved-${SLURM_JOB_ID:-local}" \
    "$@"
