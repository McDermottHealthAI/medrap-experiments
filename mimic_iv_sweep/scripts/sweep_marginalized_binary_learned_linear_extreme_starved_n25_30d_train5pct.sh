...
# capacity cut COMBINED with train-set subsampled to 5%. Pairs with
# sweep_patient_only_extreme_starved_n25_30d_train5pct.sh.
# ------------------------------------------------------------
# Uses the restored tensorized cohort and the existing train5pct
...
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/sweep_marginalized_binary_learned_linear_extreme_starved_n25_30d_train5pct.sh
# ============================================================

#SBATCH --job-name=sweep-marginalized-binary-learned-linear-extreme-starved-train5pct
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_extreme_starved_n25_30d_train5pct/draw${DRAW}"
...
echo "  Draw          : ${DRAW}/5"
echo "  query_projector: sequence_mean_1024 (learned-linear), EXTREME capacity cut, train5pct"
echo "  Started       : $(date)"
echo ""
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-binary-learned-linear-extreme-starved-train5pct-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
