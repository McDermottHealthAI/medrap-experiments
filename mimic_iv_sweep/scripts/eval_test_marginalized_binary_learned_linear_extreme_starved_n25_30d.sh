...
#              checkpoints
#              (sweep_marginalized_binary_learned_linear_extreme_starved_n25_30d.sh).
# ------------------------------------------------------------
# eval_mode=test, retriever.k=4 (matching training exactly -- no
# top1/random1 ablation here, this evaluates the model as trained). Uses
# the restored tensorized cohort (see
# sweep_marginalized_binary_learned_linear_extreme_starved_n25_30d.sh).
#
...
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/eval_test_marginalized_binary_learned_linear_extreme_starved_n25_30d.sh
# ============================================================

#SBATCH --job-name=eval-test-marginalized-binary-learned-linear-extreme-starved-n25-30d
#SBATCH --array=0-4
...
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_extreme_starved_n25_30d_test_eval/draw${DRAW}"
...
if [ ! -f "${CHECKPOINT_PATH}" ]; then
    echo "ERROR: ${CHECKPOINT_PATH} not found. Run sweep_marginalized_binary_learned_linear_extreme_starved_n25_30d.sh for draw=${DRAW} first." >&2
    exit 1
fi
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=eval-test-marginalized-binary-learned-linear-extreme-starved-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
