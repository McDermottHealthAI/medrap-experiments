...
# patient_only, train50pct checkpoints.
# ============================================================

#SBATCH --job-name=eval-test-patient-only-extreme-starved-train50pct
#SBATCH --array=0-4
...
TRAIN_OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_extreme_starved_n25_30d_train50pct/draw${DRAW}"
CHECKPOINT_PATH="${TRAIN_OUTPUT_DIR}/checkpoints/last.ckpt"
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_extreme_starved_n25_30d_train50pct_test_eval/draw${DRAW}"
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=eval-test-patient-only-extreme-starved-train50pct-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
