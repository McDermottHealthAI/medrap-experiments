...
#              query_projector=qwen3_text (MedRAP#101) instead of the
#              learned linear query projector -- same Zach
#              anchor_strategy=uniform_event 30d labels as
#              sweep_marginalized_binary_zach_uniform_event_n25_30d.sh, only
#              the query projector changes.
# ------------------------------------------------------------
# Investigating why marginalized(binary) never beats patient_only: the
# learned linear query projector starts randomly initialized with no
# mechanism to align to the frozen Qwen3-embedded retrieval doc corpus
# (FAISS search is non-differentiable, so the task loss can only reweight
# among already-retrieved docs, never teach the projector to find better
# ones). query_projector=qwen3_text instead renders each patient's recent
# codes as text and embeds them with the *same* frozen Qwen3-Embedding-0.6B
# model used to build the doc corpus -- queries and documents land in the
# same space by construction. See MedRAP@experiment/qwen3-text-query-projector
# (PR #101).
#
...
#SBATCH --job-name=sweep-marginalized-binary-qwen3-text-query-n25-30d
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_qwen3_text_query_n25_30d/draw${DRAW}"
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-binary-qwen3-text-query-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
