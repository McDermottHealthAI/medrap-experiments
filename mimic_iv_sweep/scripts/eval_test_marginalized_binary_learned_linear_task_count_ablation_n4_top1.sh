...
# for marginalized(learned-linear), N=4 tasks.
# ============================================================

#SBATCH --job-name=eval-test-marginalized-task-count-n4-top1
...
TRAIN_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_task_count_ablation_n4"
CHECKPOINT_PATH="${TRAIN_OUTPUT_DIR}/checkpoints/last.ckpt"
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_task_count_ablation_n4_test_eval_top1"
...
echo "=== Job info ==="
echo "  eval_mode     : test (MEDS held_out split), N=4 tasks, top1-only"
echo "  Started       : $(date)"
echo ""
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=eval-test-marginalized-task-count-n4-extreme-starved-top1-${SLURM_JOB_ID:-local}" \
    "$@"
