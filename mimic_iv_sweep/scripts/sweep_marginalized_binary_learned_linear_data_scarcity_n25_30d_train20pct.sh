...
#              SET subsampled to 20% of subjects (tuning/held_out are
#              full, unchanged size). Model is at FULL capacity (identical
#              to sweep_marginalized_binary_zach_uniform_event_n25_30d.sh)
#              -- pairs with
#              sweep_patient_only_data_scarcity_n25_30d_train20pct.sh,
#              which trains on the exact same subsampled label file.
# ------------------------------------------------------------
# See sweep_patient_only_data_scarcity_n25_30d_train20pct.sh for the
...
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/sweep_marginalized_binary_learned_linear_data_scarcity_n25_30d_train20pct.sh
# ============================================================

#SBATCH --job-name=sweep-marginalized-binary-data-scarcity-n25-30d-train20pct
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_data_scarcity_n25_30d_train20pct/draw${DRAW}"
...
# 20%-subsampled train split) differ here.
medrap-train \
    encoder=rope \
    pooling=masked_mean \
    head=linear \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-binary-data-scarcity-train20pct-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
