...
#              (marginalized_output_mode=binary, k=32, query_projector=
...
#SBATCH --job-name=sweep-marginalized-binary-learned-linear-capacity-starved-n25-30d-k32
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_capacity_starved_n25_30d_k32/draw${DRAW}"
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-binary-learned-linear-capacity-starved-k32-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
