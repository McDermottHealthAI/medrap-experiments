...
# capacity-sweep dim8. retriever.k=4 (as trained).
# ============================================================

#SBATCH --job-name=eval-test-marginalized-capacity-sweep-dim8
#SBATCH --array=0-4
...
TRAIN_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_capacity_sweep_n25_30d_dim8/draw${DRAW}"
CHECKPOINT_PATH="${TRAIN_OUTPUT_DIR}/checkpoints/last.ckpt"
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_capacity_sweep_n25_30d_dim8_test_eval/draw${DRAW}"
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=eval-test-marginalized-capacity-sweep-dim8-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
