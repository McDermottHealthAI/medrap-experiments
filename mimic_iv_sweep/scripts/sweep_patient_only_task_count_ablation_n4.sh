...
# (embedding_dim=4, ff_dim=8, max_seq_len=8), N=4 task codes.
# ------------------------------------------------------------
# METHODOLOGY CAVEAT: reuses the OLD label set at data/tasks/n4/tasks/
...
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/sweep_patient_only_task_count_ablation_n4.sh
# ============================================================

#SBATCH --job-name=sweep-patient-only-task-count-n4
...
OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_task_count_ablation_n4"
...
echo "=== Job info ==="
echo "  Architecture  : patient_only, EXTREME capacity cut, N=4 tasks"
echo "  Started       : $(date)"
echo ""
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=patient-only-task-count-n4-extreme-starved-${SLURM_JOB_ID:-local}" \
    "$@"
