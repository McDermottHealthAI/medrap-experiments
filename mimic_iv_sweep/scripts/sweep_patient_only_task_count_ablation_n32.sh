...
# (embedding_dim=4, ff_dim=8, max_seq_len=8), N=32 task codes.
# ------------------------------------------------------------
# METHODOLOGY CAVEAT: reuses the OLD label set at data/tasks/n32/tasks/
...
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/sweep_patient_only_task_count_ablation_n32.sh
# ============================================================

#SBATCH --job-name=sweep-patient-only-task-count-n32
...
OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_task_count_ablation_n32"
...
echo "=== Job info ==="
echo "  Architecture  : patient_only, EXTREME capacity cut, N=32 tasks"
echo "  Started       : $(date)"
echo ""
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=patient-only-task-count-n32-extreme-starved-${SLURM_JOB_ID:-local}" \
    "$@"
