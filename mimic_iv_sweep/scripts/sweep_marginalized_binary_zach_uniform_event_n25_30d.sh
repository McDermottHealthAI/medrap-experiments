...
#              generate_labels_zach_uniform_event_n25_30d.sh) -- fixed N=25,
#              5 draws, 30-day fixed occurrence window. Pairs with
#              sweep_patient_only_zach_uniform_event_n25_30d.sh -- both train
...
#SBATCH --job-name=sweep-marginalized-binary-zach-uniform-event-n25-30d
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_zach_uniform_event_n25_30d/draw${DRAW}"
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-binary-zach-uniform-event-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
