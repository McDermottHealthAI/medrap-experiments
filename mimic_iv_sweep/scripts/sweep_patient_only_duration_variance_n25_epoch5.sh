...
#              at 5 epochs instead of the usual 3 -- fixed N=25, 5
#              independent random draws, at 7-day and 30-day fixed
...
#              (k=4) -- both train on the exact same per-(duration, draw)
#              label file, so their AUROC difference is a paired
#              comparison, and both compare against the 3-epoch results in
...
#
# Identical to sweep_patient_only_duration_variance_n25.sh except
...
#SBATCH --job-name=sweep-patient-only-duration-variance-n25-epoch5
#SBATCH --array=0-9
...
OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_duration_variance_n25_epoch5/d${DURATION_DAYS}/draw${DRAW}"
...
# Same combo as sweep_patient_only_duration_variance_n25.sh -- only
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=patient-only-duration-variance-epoch5-d${DURATION_DAYS}-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
