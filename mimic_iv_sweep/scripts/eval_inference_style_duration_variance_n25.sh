#!/bin/bash
# ============================================================
# SLURM array: evaluate the k=4 duration x variance study's marginalized
#              (binary) checkpoints (sweep_marginalized_binary_duration_variance_n25.sh,
#              N=25, 5 draws, 7d/30d) held-out test AUROC in two "inference
#              style" single-document modes:
...
#              (K-agnostic marginalization, random_docs ablation).
# ------------------------------------------------------------
# 20 array tasks = 2 modes x 2 durations x 5 draws, flattened as
# IDX = mode_idx * 10 + duration_idx * 5 + draw_idx (mode_idx in [0,1]:
# 0=top1, 1=random1; duration_idx in [0,1]; draw_idx in [0,4]) -- same
# duration/draw flattening as sweep_marginalized_binary_duration_variance_n25.sh.
#
...
#
# Prerequisites:
#   sweep_marginalized_binary_duration_variance_n25.sh must have already
#   finished (checkpoints at
#   outputs/marginalized_binary_duration_variance_n25/d<D>/draw<d>/checkpoints/last.ckpt).
#
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/eval_inference_style_duration_variance_n25.sh
# ============================================================

#SBATCH --job-name=eval-inference-style-duration-variance-n25
#SBATCH --array=0-19
...
TRAIN_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_duration_variance_n25/d${DURATION_DAYS}/draw${DRAW}"
CHECKPOINT_PATH="${TRAIN_OUTPUT_DIR}/checkpoints/last.ckpt"
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_duration_variance_n25_${MODE}_eval/d${DURATION_DAYS}/draw${DRAW}"
...
if [ ! -f "${CHECKPOINT_PATH}" ]; then
    echo "ERROR: ${CHECKPOINT_PATH} not found. Run sweep_marginalized_binary_duration_variance_n25.sh for duration=${DURATION_DAYS}d draw=${DRAW} first." >&2
    exit 1
fi
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-binary-${MODE}-test-duration-variance-d${DURATION_DAYS}-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
