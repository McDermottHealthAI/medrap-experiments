...
#              30d labels, with the TRAINING SET subsampled to 10% of
...
#              sweep_marginalized_binary_learned_linear_data_scarcity_n25_30d_train10pct.sh,
...
# to learn the task, independent of parameter count? At 10% subsampling
...
#   python3 scripts/subsample_train_labels.py <source> <dest> --fraction 0.1 --seed <seed>
#     (run once per draw to produce data/tasks_zach_uniform_event_n25_30d_train10pct/draw<d>/tasks)
#
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/sweep_patient_only_data_scarcity_n25_30d_train10pct.sh
# ============================================================

#SBATCH --job-name=sweep-patient-only-data-scarcity-n25-30d-train10pct
#SBATCH --array=0-4
...
LABELS_DIR="${REPO_DIR}/data/tasks_zach_uniform_event_n25_30d_train10pct/draw${DRAW}/tasks"
OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_data_scarcity_n25_30d_train10pct/draw${DRAW}"
...
if [ ! -d "${LABELS_DIR}" ]; then
    echo "ERROR: ${LABELS_DIR} not found. Run scripts/subsample_train_labels.py for draw=${DRAW} fraction=0.1 first." >&2
    exit 1
fi
...
echo "  Duration      : 30d"
echo "  Draw          : ${DRAW}/5"
echo "  Architecture  : patient_only (no retrieval), FULL capacity, train subsampled to 10%"
echo "  Started       : $(date)"
echo ""

# Same combo as sweep_patient_only_zach_uniform_event_n25_30d.sh -- only
# LABELS_DIR/OUTPUT_DIR/wandb_run_name (pointing at the 10%-subsampled
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=patient-only-data-scarcity-train10pct-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
