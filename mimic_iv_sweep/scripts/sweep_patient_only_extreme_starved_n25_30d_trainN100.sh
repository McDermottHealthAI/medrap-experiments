...
# row count (N=100), not a percentage. Extends the extreme_starved x
...
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/sweep_patient_only_extreme_starved_n25_30d_trainN100.sh
# ============================================================

#SBATCH --job-name=sweep-patient-only-extreme-starved-trainN100
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_extreme_starved_n25_30d_trainN100/draw${DRAW}"
...
echo "=== Job info ==="
echo "  Draw          : ${DRAW}/5"
echo "  Architecture  : patient_only, EXTREME capacity cut, trainN=100"
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=patient-only-extreme-starved-trainN100-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
