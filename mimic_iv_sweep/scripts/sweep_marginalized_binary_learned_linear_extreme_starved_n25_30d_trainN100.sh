...
# capacity cut, train set subsampled to an EXACT absolute row count
# (N=100). Pairs with sweep_patient_only_extreme_starved_n25_30d_trainN100.sh.
# ------------------------------------------------------------
# Uses the restored tensorized cohort. Full retrieval corpus (no
# subsampling), K=4. max_epochs/warmup_steps capped per N (see README).
#
# 5 array tasks = 5 draws.
#
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/sweep_marginalized_binary_learned_linear_extreme_starved_n25_30d_trainN100.sh
# ============================================================

#SBATCH --job-name=sweep-marginalized-extreme-starved-trainN100
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_extreme_starved_n25_30d_trainN100/draw${DRAW}"
...
echo "=== Job info ==="
echo "  Draw          : ${DRAW}/5"
echo "  query_projector: sequence_mean_1024, EXTREME capacity cut, trainN=100"
echo "  max_epochs    : 20, warmup_steps: 15"
echo "  Started       : $(date)"
echo ""
...
medrap-train \
    ...
    training.module.warmup_steps=15 \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-extreme-starved-trainN100-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
