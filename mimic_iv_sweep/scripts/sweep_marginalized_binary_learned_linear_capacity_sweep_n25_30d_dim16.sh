...
# capacity-only cut (embedding_dim=16, ff_dim=32) with max_seq_len
# HELD FIXED at 256. Pairs with
# sweep_patient_only_capacity_sweep_n25_30d_dim16.sh.
# ------------------------------------------------------------
...
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/sweep_marginalized_binary_learned_linear_capacity_sweep_n25_30d_dim16.sh
# ============================================================

#SBATCH --job-name=sweep-marginalized-capacity-sweep-dim16
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_capacity_sweep_n25_30d_dim16/draw${DRAW}"
...
echo "=== Job info ==="
echo "  Draw          : ${DRAW}/5"
echo "  query_projector: sequence_mean_1024, embedding_dim=16, max_seq_len=256 (fixed)"
echo "  Started       : $(date)"
echo ""
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-capacity-sweep-dim16-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
