...
#              30d labels, at an EXTREME capacity cut -- one step further
#              than sweep_patient_only_capacity_starved_n25_30d.sh
#              (embedding_dim 16->4, ff_dim 32->8, max_seq_len 32->8;
#              num_heads and num_layers already at their practical floor of 1).
#              Full training data (no subsampling) -- this establishes the
#              capacity floor before building the full N x M (capacity x
#              corpus-size) grid.
# ------------------------------------------------------------
# Pairs with sweep_marginalized_binary_learned_linear_extreme_starved_n25_30d.sh.
#
# NOTE: the original tensorized cohort at
# /groups/mm6677_gp/data/MIMIC_MEDS/MEDS_cohort/processed was deleted
# ~2026-08-25. This uses the byte-for-byte snapshot-restored copy at
# /groups/mm6677_gp/zzw2102/data/MIMIC_MEDS_restored/processed instead
# (verified present, group-readable).
#
# 5 array tasks = 5 draws, matching generate_labels_zach_uniform_event_n25_30d.sh.
#
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/sweep_patient_only_extreme_starved_n25_30d.sh
# ============================================================

#SBATCH --job-name=sweep-patient-only-extreme-starved-n25-30d
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_extreme_starved_n25_30d/draw${DRAW}"
...
echo "  Duration      : 30d"
echo "  Draw          : ${DRAW}/5"
echo "  Architecture  : patient_only (no retrieval), EXTREME capacity cut"
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=patient-only-extreme-starved-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
