...
#              extreme-starved marginalized(binary, learned-linear) (train50pct)
#              checkpoints.
# ------------------------------------------------------------
# retriever.k=1, ablation_mode=none: only the single top-retrieved
# document, at eval time (valid for any checkpoint regardless of trained
# k). Compares against the k=4 (as-trained) test-eval number from
# eval_test_marginalized_binary_learned_linear_extreme_starved_n25_30d_train50pct.sh.
#
# 5 array tasks = 5 draws.
#
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/eval_test_marginalized_binary_learned_linear_extreme_starved_n25_30d_train50pct_top1.sh
# ============================================================

#SBATCH --job-name=eval-test-marginalized-extreme-starved-train50pct-top1
#SBATCH --array=0-4
...
TRAIN_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_extreme_starved_n25_30d_train50pct/draw${DRAW}"
CHECKPOINT_PATH="${TRAIN_OUTPUT_DIR}/checkpoints/last.ckpt"
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_extreme_starved_n25_30d_train50pct_test_eval_top1/draw${DRAW}"
...
echo "=== Job info ==="
echo "  Draw          : ${DRAW}/5"
echo "  query_projector: sequence_mean_1024 (learned-linear), EXTREME capacity cut, train50pct, top1-only"
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=eval-test-marginalized-extreme-starved-train50pct-top1-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
