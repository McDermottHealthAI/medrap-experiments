...
# full capacity (embedding_dim=128, the top anchor of the capacity_sweep
# line). Evaluates the pre-existing
# marginalized_binary_zach_uniform_event_n25_30d checkpoints against the
# restored tensorized cohort. retriever.k=4 (as trained).
# ============================================================

#SBATCH --job-name=eval-test-marginalized-capacity-sweep-dim128
#SBATCH --array=0-4
...
TRAIN_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_zach_uniform_event_n25_30d/draw${DRAW}"
CHECKPOINT_PATH="${TRAIN_OUTPUT_DIR}/checkpoints/last.ckpt"
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_capacity_sweep_n25_30d_dim128_test_eval/draw${DRAW}"
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=eval-test-marginalized-capacity-sweep-dim128-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
