...
# capacity-sweep dim16.
# ============================================================

#SBATCH --job-name=eval-test-patient-only-capacity-sweep-dim16
#SBATCH --array=0-4
...
TRAIN_OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_capacity_sweep_n25_30d_dim16/draw${DRAW}"
CHECKPOINT_PATH="${TRAIN_OUTPUT_DIR}/checkpoints/last.ckpt"
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_capacity_sweep_n25_30d_dim16_test_eval/draw${DRAW}"
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=eval-test-patient-only-capacity-sweep-dim16-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
