...
#              (marginalized_output_mode=binary, k=4) for the duration x
#              variance study, at 5 epochs instead of the usual 3 -- fixed
#              N=25, 5 independent random draws, at 7-day and 30-day fixed
#              occurrence-window durations (see
#              generate_labels_duration_variance_n25.sh). Pairs with
#              sweep_patient_only_duration_variance_n25_epoch5.sh -- both
#              train on the exact same per-(duration, draw) label file, and
#              both compare against the 3-epoch results in
#              sweep_marginalized_binary_duration_variance_n25.sh /
...
# sweep_patient_only_duration_variance_n25_epoch5.sh.
#
# Identical to sweep_marginalized_binary_duration_variance_n25.sh except
# training.trainer.max_epochs=5 (and OUTPUT_DIR/wandb_run_name reflect
# that). k stays at 4 -- this is purely an epoch-count comparison, not a
# k sweep (see sweep_marginalized_binary_duration_variance_n25_large_k.sh
# for that).
#
# Prerequisites:
...
#SBATCH --job-name=sweep-marginalized-binary-duration-variance-n25-epoch5
#SBATCH --array=0-9
...
OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_duration_variance_n25_epoch5/d${DURATION_DAYS}/draw${DRAW}"
...
# max_epochs, OUTPUT_DIR, and wandb_run_name differ here.
medrap-train \
    encoder=rope \
    pooling=masked_mean \
    head=linear \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-binary-duration-variance-epoch5-d${DURATION_DAYS}-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
