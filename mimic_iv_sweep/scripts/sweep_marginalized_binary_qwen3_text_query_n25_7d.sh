...
#              anchor_strategy=uniform_event 7d labels as
#              sweep_marginalized_binary_zach_uniform_event_n25_7d.sh, only
...
#SBATCH --job-name=sweep-marginalized-binary-qwen3-text-query-n25-7d
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_qwen3_text_query_n25_7d/draw${DRAW}"
...
# Same combo as sweep_marginalized_binary_zach_uniform_event_n25_7d.sh --
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-binary-qwen3-text-query-d7-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
