...
#SBATCH --job-name=eval-test-patient-only-capacity-starved-n25-30d
#SBATCH --array=0-4
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=patient-only-capacity-starved-test-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
