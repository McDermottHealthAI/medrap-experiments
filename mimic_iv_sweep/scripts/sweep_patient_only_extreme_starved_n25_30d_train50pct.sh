...
# ff_dim=8, max_seq_len=8) COMBINED with train-set subsampled to 50%.
...
# Reuses the train50pct subsampled labels from the data-scarcity line
# (data/tasks_zach_uniform_event_n25_30d_train50pct/) -- no new
# subsampling needed.
#
...
# Usage:
#   cd mimic_iv_sweep
#   sbatch scripts/sweep_patient_only_extreme_starved_n25_30d_train50pct.sh
# ============================================================

#SBATCH --job-name=sweep-patient-only-extreme-starved-train50pct
#SBATCH --array=0-4
...
OUTPUT_DIR="${REPO_DIR}/outputs/patient_only_extreme_starved_n25_30d_train50pct/draw${DRAW}"
...
echo "  Draw          : ${DRAW}/5"
echo "  Architecture  : patient_only, EXTREME capacity cut, train50pct"
echo "  Started       : $(date)"
echo ""
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=patient-only-extreme-starved-train50pct-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
