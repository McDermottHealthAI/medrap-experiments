...
#              marginalized_output_mode=binary, trained on RANDOM CODE
#              SELECTION + RANDOM PER-TASK DURATION labels
#              (generate_labels_duration_n1248.sh) instead of the fixed
#              7-day-horizon random-task labels
#              sweep_marginalized_binary_n1248.sh uses, N in
#              {1, 2, 4, 8, 16, 32, 64, 128}.
# ------------------------------------------------------------
# Identical architecture/hyperparameters to sweep_marginalized_binary_n1248.sh
# -- only task_labels_dir differs (data/tasks_duration/n<N>/tasks instead of
# data/tasks/n<N>/tasks), so this isolates "random codes + random durations"
# against "random codes + fixed 7-day duration" (marginalized-binary-n{N}-*)
# on the same architecture. See also
# ../mimic_iv_sweep_frequent/scripts/sweep_marginalized_binary_duration_n1248.sh
# for the most-frequent-code counterpart of this same random-duration idea.
#
# Array index -> N:
#   0 -> N=1
#   1 -> N=2
...
#SBATCH --job-name=sweep-marginalized-binary-duration-n1248
#SBATCH --array=0-7
...
if [ ! -d "${LABELS_DIR}" ]; then
    echo "ERROR: ${LABELS_DIR} not found. Run sbatch scripts/generate_labels_duration_n1248.sh for N=${N} first." >&2
    exit 1
fi
...
medrap-train \
    ...
    do_overwrite=true \
    "wandb_run_name=marginalized-binary-duration-n${N}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
