#!/bin/bash
# ============================================================
# SLURM array: held-out TEST split AUROC, TOP-1 DOC ONLY, for
# marginalized(learned-linear), extreme-starved, trainN=100000.
# retriever.k=1, ablation_mode=none -- matches the convention already
# established in results/extreme_capacity_starved_retrieval/README.md for
# the 100/50/5pct data-fraction levels.
# ============================================================

#SBATCH --job-name=eval-test-marginalized-extreme-starved-trainN100000-top1
#SBATCH --array=0-4
...
LABELS_DIR="${REPO_DIR}/data/tasks_zach_uniform_event_n25_30d_trainN100000/draw${DRAW}/tasks"
TRAIN_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_extreme_starved_n25_30d_trainN100000/draw${DRAW}"
CHECKPOINT_PATH="${TRAIN_OUTPUT_DIR}/checkpoints/last.ckpt"
EVAL_OUTPUT_DIR="${REPO_DIR}/outputs/marginalized_binary_learned_linear_extreme_starved_n25_30d_trainN100000_test_eval_top1/draw${DRAW}"
...
echo "=== Job info ==="
echo "  Draw          : ${DRAW}/5"
echo "  eval_mode     : test (MEDS held_out split), top1-only"
echo "  Started       : $(date)"
echo ""
...
medrap-eval \
    ...
    do_overwrite=true \
    "wandb_run_name=eval-test-marginalized-extreme-starved-trainN100000-top1-d30-draw${DRAW}-${SLURM_ARRAY_JOB_ID:-local}_${IDX}" \
    "$@"
